"""Evaluate selected embedding on the unchanged original Fig.10 grid and repetitions.
Run benchmark_selected.py first. Original results/minimal_revision is never overwritten.
"""
...
def run(name):
 dest=BASE/'results/optimized_revision';dest.mkdir(parents=True,exist_ok=True)
 c=EmbedConfig(**json.loads((BASE/'results/optimized_revision/config.json').read_text()));w=load_watermark(BASE/'data/M.png')
 saved=BASE/'outputs/optimized_revision'/f'{name}.shp'
 if saved.exists():
...
if __name__=='__main__':
 ...
 dest=BASE/'results/optimized_revision';f=pd.concat([pd.read_csv(dest/f'{n}.csv') for n in args.datasets],ignore_index=True);f.to_csv(dest/'raw.csv',index=False)
